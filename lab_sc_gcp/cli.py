...
def create_parser():
    ...
    # Upload 10x libraries to bucket parser
    parser_upload_10x = subargs.add_parser(
        c_UPLOAD_10X,
        help="Upload one or more 10x libraries to default bucket.",
    )
    parser_upload_10x.add_argument(
        '--user',
        default=USER,
        help='User name to associate with instance.',
    )
...
def main():
    ...
    if parsed_args.command == c_UPLOAD_10X:
        ...
        if os.path.isdir(parsed_args.libraries):
        ...
        upload_libraries(libraries=libraries,
